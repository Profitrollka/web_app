...
class Service(object):
    ...
    def _isinstance(self, model, raise_error=True) -> bool:
    ...
    def get_order_by(self, value_for_sort, sort_order=None, number_of_values_for_receive=None):
        """Returns a sorted instances of the service's model.
        Returns `None` if an instance with the specified id does not exist
        :param value_for_sort: column name for sort results
        :param sort_order: string value for sorting order('desc' or 'asc')
        :param number_of_values_for_receive: number of values for receive
        """
        if sort_order == 'desc':
            if number_of_values_for_receive:
                return self.__model__.query.order_by(desc(value_for_sort))\
                    .limit(number_of_values_for_receive)
            else:
                return self.__model__.query.order_by(desc(value_for_sort))
        else:
            if number_of_values_for_receive:
                return self.__model__.query.order_by(value_for_sort)\
                    .limit(number_of_values_for_receive)
            else:
                return self.__model__.query.order_by(value_for_sort)
